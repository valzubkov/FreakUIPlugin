...
- Settings is built into every generated foundation and is not an app-specific top-level screen. If the user includes Settings among the main sections, preserve that intent by omitting it from `topLevelScreens`, confirming `Settings: Included` separately, and keeping two to five other app-specific sections. Ask for another app-specific section only when removing Settings would leave fewer than two.
